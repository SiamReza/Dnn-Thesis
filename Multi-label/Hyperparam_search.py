...
from Evaluation_Matix import get_AUC
...
def main():
	...
	for network in netlist:
		...
		CV_iter = (0, 1)
		Best_AUC = 0
		for hyperparam in param_search_list(hyperparams):
			logging.warning('testing hyperparameter {}'.format(hyperparam))
			params.hyperparam = copy(hyperparams)
			params.hyperparam.update(hyperparam)
			solver = Solver(args, params, CV_iter)
			
			if args.train:
				solver.train()
			
			
			AUC = get_AUC(solver.validate('test'))
			if AUC > Best_AUC:
				Best_AUC = AUC
				hyperparams.update(hyperparam)
				hyperparams.save()
