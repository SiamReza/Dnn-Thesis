...
def get_eval_multi(outputs):
	...
	outputs[0] = np.argmax(outputs[0], axis=1)
	outputs[1] = np.argmax(outputs[1], axis=1)
	
	result['acc'] = metrics.accuracy_score(outputs[1], outputs[0])
	result['Precision'] = metrics.precision_score(outputs[1], outputs[0], average='weighted', zero_division = 0)
	result['Recall'] = metrics.recall_score(outputs[1], outputs[0], average='weighted')
	result['F0.5'] = metrics.fbeta_score(outputs[1], outputs[0], average='weighted', beta=0.5)
	result['F0'] = metrics.fbeta_score(outputs[1], outputs[0], average='weighted', beta=0)
	result['F1'] = metrics.f1_score(outputs[1], outputs[0], average='weighted')
